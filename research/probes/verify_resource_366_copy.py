"""Verify complete366 DEV copy and rerun its frozen classifier, without a solver.

Only host/filesystem adapters change. No metric, evidence or frozen code changes.
"""
import hashlib
...
COPY=ROOT/'artifacts/research/366/completed'
...
def extract_once():
    archive=COPY.parent/'complete-development.tar.gz'
    assert sha(archive)=='CF96CF19F1FA2C4A3FC2BC995488263678A643A2844AE0018DC122EE075B1D4E'
    if COPY.exists():return
    with tarfile.open(archive) as tf:
        members=tf.getmembers()
        assert len(members)==1059 and len({m.name for m in members})==1059
        for m in members:
...
def verify_copy():
    expected={'input366.json':'5447CE02A11D6803B0EB9878A84B6B343A4E2EBA8E2D1EFAD6F9A6C6615A5B8F',
        ...
        f'research/evidence/{ID}-development.summary.json':'5C31F1FFE142675563171FCA5FC6A7C432F08BD42BE0D9ED5EE541241AA0A376',
        f'research/evidence/{ID}-development/run_complete.json':'62E584047290605FE72C83B0AE4129BD36EA7E789866F324B56F481038CA8562'}
    ...
    assert sum(r['actions'] for r in rows)==912 and sum(r['transitions'] for r in rows)==768
    assert all(r['failure'] is None for r in rows)
    assert all(p.stat().st_size==0 for p in DATA.rglob('*.stderr'))
    assert (COPY/'runner366.stderr').stat().st_size==0
    return m
...
def main():
    ...
    spec=importlib.util.spec_from_file_location('frozen366_copy',COPY/'score_resource_366.py')
    ...
    report=load(COPY/'research/evidence'/(ID+'-development.summary.json'))
    rows=report['rows'];inactive=[r for r in rows if r['window_ms']==5000]
    frames=[x for r in rows for x in r['certificate']['frames']]
    audit={'verified':True,'files':len(load(DATA/'run_complete.json')['files']),
        'results':144,'fixtures':36,'acks':912,'transitions':768,
        'summary_sha256':sha(COPY/'research/evidence'/(ID+'-development.summary.json')),
        ...
        'same_binary_controls':report['same_binary_controls'],
        'robust_intervals':report['robust_intervals'],
    ...
    print('verified366',json.dumps({k:audit[k] for k in ('files','results','fixtures','acks','transitions','active','inactive','work')}))
