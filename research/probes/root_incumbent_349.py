...
ID = 'ATTR-ROOT-INCUMBENT-CERTIFICATE-349'
...
def freeze_inputs():
    ...
    paths = set(m['hashes']) | {str(p.relative_to(ROOT)),
        ...
        'research/probes/root_incumbent_349.py',
...
def freeze_execution():
    ...
    paths=set(m['hashes'])|{str(M.relative_to(ROOT)),
        'artifacts/research/349/horizon_pricing.cpp','artifacts/research/349/probe.cpp',
        'artifacts/research/349/probe.exe','research/probes/build_root_incumbent_349.cmd',
        'research/probes/test_root_incumbent_349.py'}
...
def validate_observed(c, out, judge):
    ...
    require(len(set(obs['pair']))==2, 'incomplete pair')
...
def run():
    ...
    D.mkdir()
    probe=Bridge(ROOT/'artifacts/research/349/probe.exe')
    ...
    verify(e)
    write_new(D/'run_complete.json',{'cases':24,'queries':216,'control_queries':216,
...
def summarize():
    ...
    require(counts['queries']==216,'total queries')
