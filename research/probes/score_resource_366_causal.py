"""366-P2 one-pass measurement adapter; frozen P1 runtime and validators unchanged.
...
EXEC = PACKAGE/'execution366-causal.json'
ID = 'SCORE-CAUSAL-RESOURCE-QUALIFICATION-366'
DATA = WORK/'research/evidence'/(ID+'-protected-causal-single')
SUMMARY = WORK/'research/evidence'/(ID+'-protected-causal-single.summary.json')
...
def require_previous(summary,completion):
    ...
    require(summary['zero_safety_failure'] is True,'holdout safety failure')
    failed = {k for k,v in summary['gate']['checks'].items() if not v}
    require(failed == {'inactive_upper_tiers','B_overall_components'},
            'unexpected prior failure; user amendment is not a general waiver')
    require(summary['gate']['checks']['causal_inactive_operation_equivalence'] is True,
            'inactive proof failure')
...
def previous_gate(original,check=False):
    ...
    require(sha(WORK/'research/evidence'/(ID+'-holdout.summary.json')) ==
            '88E7FF81955C7AFAC838D430508809B8B21C2AC00C9906E7BA6E3994D9EF00B3',
            'completed original holdout summary drift')
    require(sha(complete_path) ==
            'EE02542F975343DC8E202C343EFD1F6EC57C2685F714994A076CEB71F65291A4',
            'completed original holdout marker drift')
...
def summarize(check=False):
    ...
    from gate_resource_366_causal import classify
    rows = []
    for case in cases:
    ...
    report = {'experiment':ID,'phase':'protected-causal-single','complete':True,**expected,
        ...
        'authority':'366-P2 user-directed causal decision amendment after original holdout; single-pass protected; not original gate pass or product promotion'}
...
def run(resume=False):
    ...
    print('run_complete protected-causal-single',flush=True)
    summarize()
