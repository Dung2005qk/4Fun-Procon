...
def main():
    require(not (ROOT/'artifacts/research/347/source').exists(),'source already exists')
    ...
    paths.update({'research/probes/freeze_claim_cap_347.py','research/probes/freeze_complete_pool_344.py',
        f'research/evidence/{ID}-preregistration.md','research/evidence/ATTR-CURRENT-FLOOR-CONDITIONAL-CEILING-346-closure.md',
        ...
        for phase,offset,count in (('development',0,4),('holdout',100000,9)):
            ...
            for f,family in enumerate(FAMILIES):
                ...
                while r<count:
                    ...
                    spec={'seed':20260908347000+offset+10000*f+cursor,'family':family,'players':8+(f+r)%3};cursor+=1
            ...
            for fi,fuel in enumerate(('low','default','high')):
                for si,side in enumerate((8,32)):
                    for wi,window in enumerate((5000,10000,15000)):
                        ...
                        while True:
                            require(cursor<10000,'protected identities exhausted');base_seed=202609083470000
    ...
    m={'experiment':ID,'parent_commit':'c76a8eaa4f200e3eeeb1a58ef1d1fb3d0c13579c','parent_btc_binary':base['parent_btc_binary'],
        'bridge_binary':base['bridge_binary'],'splits':splits,'limits_per_witness':base['limits_per_witness'],
