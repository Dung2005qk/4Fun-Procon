"""Create-only fresh inputs, no planner calls and no score inspection."""
...
ID = 'SCORE-COMPLETE-W1-POOL-PAIR-PRICING-344'


def order_for(index):
    labels = ['parentA', 'candidateA', 'candidateB', 'parentB']
    i = index % 4
    return labels[i:] + labels[:i]


def main():
    require(not (ROOT/'artifacts/research/344/source').exists(), 'source already exists')
    base = load(ROOT/'research/holdouts/SCORE-W1-STOCK-RELAXED-PAIR-PRICING-341.json')
    for p,h in base['parent_hashes'].items():
        require(digest(ROOT/p) == h, 'baseline dependency drift:'+p)
    seen = set()
    identity_sources = []
    names = ['ATTR-THREE-PATROL-HTTP-BASELINE-314', 'ATTR-W1-HORIZON-PRICING-PREVALENCE-331',
             ...
             'SCORE-W1-BOUNDED-PAIR-PRICING-338-development',
             *['SCORE-W1-STOCK-RELAXED-PAIR-PRICING-341-'+p for p in ('development','holdout','protected')],
             'ATTR-INACTIVE-RUNTIME-AA-CONTROL-342']
    for name in names:
        p = ROOT/f'research/holdouts/{name}.json'
        obj = load(p)
        setups = obj['setups'] if 'setups' in obj else [c['setup'] for c in obj['cases']]
        seen.update(gameplay_hash(s) for s in setups)
        identity_sources.append(str(p.relative_to(ROOT)).replace('\\','/'))
    paths = set(base['parent_hashes']) | set(identity_sources) | {
        'research/probes/freeze_complete_pool_344.py',
        f'research/evidence/{ID}-preregistration.md',
        'research/evidence/ATTR-W1-COMPLETE-PREFIX-HEADROOM-343-closure.md',
        'research/evidence/ATTR-INACTIVE-RUNTIME-AA-CONTROL-342-closure.md',
        'artifacts/research/332/protected-fixture.exe'}
    splits = {}
    bridge = Bridge(ROOT/base['bridge_binary'])
    try:
        for phase,offset,count in (('development',0,4),('holdout',100000,9)):
            ...
            for f,family in enumerate(FAMILIES):
                r=0;cursor=0
                while r<count:
                    require(cursor<10000,'fresh identity pool exhausted')
                    spec={'seed':20260907344000+offset+10000*f+cursor,'family':family,'players':8+(f+r)%3}
                    cursor+=1
                    ...
                    cases.append({'spec':spec,'setup':q['setup'],'gameplay_sha256':h,
                                  'order':['parent','candidate'],'run_order':order_for(len(cases))})
                    r+=1
            p=ROOT/f'research/holdouts/{ID}-{phase}.json'
            write_new(p,{'experiment':ID,'phase':phase,'cases':cases,'identity_skips':skips,
                         'repetitions':2,'authority':'Synthetic actual WinHTTP, not official BTC.'})
            ...
            for fi,fuel in enumerate(('low','default','high')):
                for si,side in enumerate((8,32)):
                    for wi,window in enumerate((5000,10000,15000)):
                        ...
                        while True:
                            require(cursor<10000,'protected identity pool exhausted')
                            base_seed=202609073440000
                            q={'seed':base_seed-base_seed%6+6*n+f+cursor*6000,'side':side,'fuel':fuel,
                               'window_ms':window,'days':(4,5,10)[(f+fi+wi)%3],'players':8+(f+wi)%3,
                               'roadless':(f+fi+si)%2==0}
                            cursor+=1
                            result=bridge.request(q);require(result.get('ok'),'invalid protected fixture')
                            h=gameplay_hash(result['setup'])
                            if h in seen:skips.append(q['seed']);continue
                            seen.add(h);break
                        cases.append({**q,**result,'gameplay_sha256':h,
                                      'role':'fixed-all-Patrol' if (f+wi)%2==0 else 'native',
    ...
    m={'experiment':ID,'parent_commit':'c76a8eaa4f200e3eeeb1a58ef1d1fb3d0c13579c',
       'parent_btc_binary':base['parent_btc_binary'],'bridge_binary':base['bridge_binary'],
       'splits':splits,'limits_per_witness':base['limits_per_witness'],'resource_floor_bytes':1073741824,
       'policy':f'research/evidence/{ID}-preregistration.md','hashes':{p:digest(ROOT/p) for p in sorted(paths)},
       'stage':'Inputs frozen before source; no score measurement until execution contract/tests frozen',
       'sealed_opened':False,'production_change':False}
    p=ROOT/f'research/holdouts/{ID}.json';write_new(p,m)
    print({'manifest_sha256':digest(p),'splits':splits})
