...
from freeze_complete_pool_344 import ID, order_for

M=ROOT/f'research/holdouts/{ID}-execution-ram-policy.json'
...
def validate(c,label,directory,phase):
    ...
    row=(protected_validate if phase=='protected' else small_validate)(c,side,base)
    if side=='candidate':
...
def coverage(cases,phase):
    n={'development':24,'holdout':54,'protected':108}[phase]
    ...
    try:
        for i,c in enumerate(cases):
            for label in c['run_order']:
                ...
                if phase=='protected':protected_run(c,m[side+'_binary'],p.parent,bridge,ID,side)
...
def summarize(phase):
        ...
        for repeat in ('A','B'):
            ...
            row={**fields,'repeat':repeat,'parent':b,'candidate':a,'comparison':outcome,'first_tier':tier,
                ...
                'safety':{side:(operational(location(c,side+repeat,directory).with_suffix('.replay.jsonl'),c,side)
                    if phase=='protected' else safety(location(c,side+repeat,directory).with_suffix('.replay.jsonl')))
                    for side in ('parent','candidate')},
    ...
    gate=protected_gate(rows,robust) if phase=='protected' else narrow_gate(rows,phase)
...
if __name__=='__main__':
    ...
    parser.add_argument('--phase',choices=('development','holdout','protected'),default='development')
