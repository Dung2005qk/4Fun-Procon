...
import anytime_score_core_350 as core
...
def summarize(phase,check=False):
    ...
    for c in cases:
        for label in core.LABELS:
            ...
            if phase not in ('protected','broad-development'):
    ...
    def write(path,value):
        value['report_contract']='Frozen350 exact roadless normalization; explicit anytime completion schema; no measurement correction.'
...
def main():
    p=argparse.ArgumentParser();p.add_argument('mode',choices=('run','summarize'));p.add_argument('--phase',choices=('development','broad-development','holdout','protected'),default='development')
