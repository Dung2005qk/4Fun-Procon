"""One-time frozen341 holdout; same holdout runtime, comparator and safety."""
import argparse
...
M = ROOT / f"research/holdouts/{ID}-holdout-execution.json"
D = ROOT / f"research/evidence/{ID}-holdout"
S = ROOT / f"research/evidence/{ID}-holdout.summary.json"
...
def run(resume=False):
    m = load(M); verify(m)
    cases = load(ROOT/m["holdout"])["cases"]
    require(len(cases) == 54, "wrong holdout coverage")
    if D.exists():
    ...
    verify(m)
    write_new(D/"run_complete.json", {"pairs": 54, "results": 108, "actions": 432, "transitions": 324,
        "execution_sha256": digest(M), "pair_hashes": {p.name: digest(p) for p in sorted(D.glob("*.pair_complete.json"))}})
    print("run_complete pairs=54 results=108", flush=True)
    summarize()
...
def summarize():
    m = load(M); verify(m); cases = load(ROOT/m["holdout"])["cases"]
    marker = load(D/"run_complete.json")
    require(marker["execution_sha256"] == digest(M) and marker["pairs"] == len(cases) == 54, "completion identity")
    require(len(list(D.glob("*.pair_complete.json"))) == 54 and
            all(len(list((D/side).glob("*.result.json"))) == 54 for side in ("parent", "candidate")), "wrong exact file counts")
    ...
    report = {"experiment": ID, "phase": "holdout", "complete": True, "pairs": 54,
        "actions": 432, "transitions": 324, "zero_safety_failure": True, "gate_passed": gate(rows),
