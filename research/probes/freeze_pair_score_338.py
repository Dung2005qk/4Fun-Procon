"""Generate/hash unplayed splits before any338candidate edit. No solver calls."""
import json
...
ID = 'SCORE-W1-BOUNDED-PAIR-PRICING-338'


def main():
    require(not (ROOT/'artifacts/research/338/source').exists(), 'candidate already exists')
    s337 = ROOT/'research/evidence/ATTR-W1-PAIR-PRICING-PREVALENCE-337.summary.json'
    require(digest(s337) == '3966E31DB7FC65673BF31E94343890BCA201935B598C3EC803F8E06D080DCF2B', '337summary drift')
    require(load(s337)['gate_passed'], '337 failed')
        ...
        for phase,offset,count in (('development',0,4),('holdout',100000,9)):
            ...
            for f,family in enumerate(FAMILIES):
                ...
                while r<count:
                    ...
                    spec={'seed':20260907338000+offset+10000*f+cursor,'family':family,'players':8+(f+r)%3}
                    cursor+=1
            ...
            for fi,fuel in enumerate(('low','default','high')):
                for si,side in enumerate((8,32)):
                    for wi,window in enumerate((5000,10000,15000)):
                        n=len(protected); base=202609073380000
    ...
    paths=set(old['parent_hashes'])|{'research/probes/freeze_pair_score_338.py','research/probes/pair_pricing_prevalence_337.py',
    ...
    m={'experiment':ID,'parent_commit':old['parent_commit'],'splits':splits,
        ...
        'mechanism':'Every unordered pair from original full W1; canonical resource frontiers; joint stock-capped reward; physical pair cell/fuel/lifetime DP; all other whole trajectories fixed. No individual-score pruning.',
